...
class ABU_ENQUEUE_FILES {
  ...
  public static function abu_admin_enqueue_scripts() {
      ...
      wp_enqueue_style( 'jquery-ui', abu_load_file( 'vector\jquery-ui', ABU_CSS, 'css' ), [] , time(), false );
      wp_enqueue_style( 'abu-fa', 'https://use.fontawesome.com/releases/v5.8.1/css/all.css', [] , '5.8.1', false );
      wp_enqueue_style( 'abu-framework-plugins', abu_load_file( 'abu-plugins', ABU_CSS, 'css' ), [] , time(), false );
      wp_enqueue_style( 'abu-framework-css', abu_load_file( 'abu-framework', ABU_CSS, 'css' ), [] , time(), false );
      ...
      wp_enqueue_script( 'abu-framework-plugins', abu_load_file( 'abu-plugins', ABU_JS, 'js' ), [], time(), false );
      wp_enqueue_script( 'acolorpicker', abu_load_file( '\vectors\acolorpicker', ABU_JS, 'js' ), [], time(), false );
      wp_enqueue_script( 'wp-color-picker-alpha', abu_load_file( '\vectors\wp-color-picker-alpha', ABU_JS, 'js' ), ['wp-color-picker'], time() );
      wp_enqueue_script( 'checkboxradio-ui', abu_load_file( '\vectors\checkboxradio-ui', ABU_JS, 'js' ), [], time() );
        ...
        foreach( AFW::$frameworks['abu_all_sections'] as $section ) {
          if( isset( $section['fields'] ) ) {
            foreach( $section['fields'] as $field ) {
              if( ! empty( $field['type'] ) ) {
                $classname = 'abuFrameworkField_' . $field['type'];
                if( class_exists( $classname ) && method_exists( $classname, 'wp_enqueue' ) ) {
                  $instance = new $classname( $field );
                  if( method_exists( $instance, 'wp_enqueue' ) ) {
                    $instance->wp_enqueue();
                  }
                  unset( $instance );
                }
